...
# ============================================================
#  🍣 OKSushi - 전문 배포 시스템 (Full Automation)
# ============================================================
...
echo -e "${BLUE}=======================================================${NC}"
echo -e "${BLUE}   🍣 OKSushi 배포 파이프라인 가동 (v2.0) ${NC}"
echo -e "${BLUE}=======================================================${NC}"
...
# limit 숫자를 조절하여 생성 개수를 제어할 수 있습니다.
...
# 5. GCS 이미지 동기화 (Static Assets serving)
echo -e "☁️  [5단계] Google Cloud Storage 이미지 동기화 중..."
# app/static/images 폴더의 이미지를 클라우드 버킷으로 전송
gsutil -m rsync -r app/static/images/ gs://${BUCKET_NAME}/${SERVICE_NAME}/
echo -e "${GREEN}✅ GCS 동기화 완료 (Bucket: ${BUCKET_NAME}/${SERVICE_NAME})${NC}"
...
if [ -d .git ]; then
    ...
    echo -e "${RED}⚠️ Git 저장소가 설정되지 않았습니다. (GitHub 업데이트 생략)${NC}"
fi

# 7. Google Cloud Build & Run Deploy
echo -e "🏗️  [7단계] Google Cloud Build 시작..."
# Artifact Registry 및 Cloud Run 배포 수행
